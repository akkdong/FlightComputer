...
extern HID_StateMachine hid_sm;
...
{
  switch (hid_sm.mouse_state)
  {
  case HID_MOUSE_IDLE:
    hid_sm.mouse_state = HID_MOUSE_START;
  ...
  case HID_MOUSE_WAIT:
    if (hid_sm.select != prev_select)
    {
      prev_select = hid_sm.select;
      //HID_SelectItem(DEMO_MOUSE_menu, hid_sm.select & 0x7F);

      /* Handle select item */
      if (hid_sm.select & 0x80)
      {
        hid_sm.select &= 0x7F;
        switch (hid_sm.select)
        {
        case 0:
          hid_sm.mouse_state = HID_MOUSE_START;
          break;
        ...
        case 1:                /* Return */
          ...
          hid_sm.state = HID_APP_REENUMERATE;
          hid_sm.select = 0;
          break;
  ...
  case HID_MOUSE_START:
    HID_MOUSE_Init();
    hid_sm.mouse_state = HID_MOUSE_WAIT;
...
/**
  ...
  * @retval None
  */
void HID_MOUSE_ProcessData(HID_MOUSE_Info_TypeDef * data)
{
