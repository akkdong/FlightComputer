...
BMP280::BMP280(SPI_HandleTypeDef *handle)
{
	...
	p_reference = 1013.25; // 1013.25 hPa
}
#else
BMP280::BMP280(SPIClass& _spi, int _csPin) : spi(_spi), csPin(_csPin)
{
	p_reference = 1013.25; // 1013.25 hPa
}
#endif
...
uint8_t BMP280::initialize()
{
	...
	setFilterCoefficient(filter_coeff_16); // filter_coeff_16
	setStandbyTime(standby_time_62500us);
...
void BMP280::measure()
{
	...
	measurement.pressure = (float) compensate_pressure(adc_P) / 256.0 / 100.0;

	if (p_reference > 0)
	{
		// 0.1902949572 = 1 / 5.255
		// 4433076.0 = (288.15 / 0.0065) * 100  : 44330.769230769230769230769230769

		measurement.altitude = (1.0
				- pow(measurement.pressure / p_reference, 0.1902949572)) * (288.15 / 0.0065); // 4433076.0
	}
}
