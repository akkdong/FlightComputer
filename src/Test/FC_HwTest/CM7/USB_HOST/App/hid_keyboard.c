...
{
  switch (hid_sm.keyboard_state)
  {
  case HID_KEYBOARD_IDLE:
    hid_sm.keyboard_state = HID_KEYBOARD_START;
  ...
  case HID_KEYBOARD_WAIT:
    if (hid_sm.select != prev_select)
    {
      ...
      //HID_SelectItem(DEMO_KEYBOARD_menu, hid_sm.select & 0x7F);
      /* Handle select item */
  ...
  case HID_KEYBOARD_START:
    HID_KEYBOARD_Init();
    hid_sm.keyboard_state = HID_KEYBOARD_WAIT;
    break;
